  ...
  return (
    <nav className="sticky top-0 bg-white z-20">
      <div className="flex relative max-w-frame mx-auto items-center justify-between py-5 md:py-6 px-4 xl:px-0">
          ...
          <div className="flex space-x-6">
            {data.map((item) => (
              <React.Fragment key={item.id}>
                {item.type === "MenuItem" && (
                  <Link href={item.url || "#"} className="text-sm font-medium hover:text-black/60 transition-all">
                    {item.label}
                  </Link>
                )}
                {item.type === "MenuList" && (
                  <div className="relative group">
                    <button className="text-sm font-medium hover:text-black/60 transition-all">
                      {item.label}
                    </button>
        ...
        <InputGroup className="hidden md:flex bg-[#F0F0F0] mr-3 lg:mr-5 max-w-[250px] lg:max-w-[300px]">
          <InputGroup.Text>
            <Image
              priority
          ...
          <InputGroup.Input
            ...
            className="bg-transparent placeholder:text-black/40 text-sm py-2"
          />
        </InputGroup>
